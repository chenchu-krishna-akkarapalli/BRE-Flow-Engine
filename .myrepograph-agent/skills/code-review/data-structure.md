...
This document outlines the data structure selection and optimization strategy engineered specifically to meet FlowBRE's strict performance SLA latency benchmarks defined in [Backend-Playbook.md](file:///c:/Projects/onboarding-bre-engine/Backend-Playbook.md):
...
- **Request Starts**: Incoming HTTP request triggers route parsing ($< 1\text{ ms}$).
- **Allocate Memory**: Transient payload objects and Pydantic models are allocated on CPython's private heap.
- **Use Memory**: $O(1)$ Hash Table lookups and pre-warmed DB pools process candidate parameters against compiled decision rules in RAM.
...
## 2. Core Data Structure & Connection Pool Strategy
...
### 🏊 2. Pre-Warmed Connection Pools — SQLAlchemy `asyncpg`
- **Use Case**: Asynchronous PostgreSQL interaction for application persistence and audit logging.
- **Connection Tuning**: `pool_size=20`, `max_overflow=10`, `pool_recycle=3600`, `pool_pre_ping=True`.
- **Single-Transaction Writes**: Application state updates and audit log writes execute in a single DB transaction to eliminate network round-trip overhead.

### ⚡ 3. Dynamic Arrays (Python Lists & Tuples) — Contiguous RAM Cache
...
### 🧠 4. In-Memory Caching (LRU Cache & Redis Key-Value Store)
...
| Data Structure / Layer | Operation | Time Complexity | Memory Impact | Target SLA Alignment |
|---|---|---|---|---|
| **Hash Table (`dict`)** | Rule / Bank Parameter Lookup | $O(1)$ | Low (Shared pre-compiled RAM) | **Simple GET < 30 ms** |
| **LRU Cache** | Pincode / Metadata Resolution | $O(1)$ | Bounded (Fixed cache budget) | **Simple GET < 30 ms** |
| **`asyncpg` Pool** | Single-Transaction DB Persistence | $O(1)$ connection reuse | Managed connection pool | **CRUD < 80 ms** |
| **Dynamic Array (`list`)** | Bureau DPD Array Scanning | $O(N)$ ($N \le 36$) | Minimal (Contiguous allocation) | **CRUD < 80 ms** |
...
1. **Zero Hot-Path Synchronous Disk I/O**: Disallow file reading, synchronous I/O, or unindexed database queries during request execution hot paths.
2. **Avoid Per-Request Re-Compilation**: Never parse JSON rule files inside an endpoint handler. Pre-compile into RAM at application boot so `Allocate Memory` costs $O(1)$ per request.
3. **Minimize Object Creation**: Pass scalar values or lightweight Pydantic v2 slots instead of instantiating deep nested object trees during evaluation.
4. **Prompt Reference Cleanup**: Ensure intermediate evaluation dicts drop out of scope immediately after evaluation so reference counting triggers instant cleanup without waiting for full generational GC sweeps.