...
2. **Load Instructions**: Load `RULES.md`, `DUTIES.md`, `SOUL.md` into the instruction layer, including performance SLAs (Simple GET `< 30 ms`, CRUD `< 80 ms`, Zen-Engine `< 10 ms`, Total `< 100 ms`) and 5-stage Memory Lifetime flow.
3. **Verify Environment & Rule Graph Readiness**: Confirm `zen_rules/*.json` decision graphs are compiled in RAM and database connection pool settings (`pool_size=20`, `max_overflow=10`) are configured.
